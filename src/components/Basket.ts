import { IBasket, IStoreItemBasketActions, IProductBasket } from '../types/Basket';
...
export class Basket extends Component<IBasket> {
  ...
  constructor(
    ...
    if (this._button) {
      this._button.addEventListener('click', () => this.events.emit(settings.events.basket.order))
    }
  }
  ...
  refreshIndices() {
    Array.from(this._list.children).forEach(
      (item, index) =>
      (item.querySelector(settings.page.basketItemIndex)!.textContent = (
        index + 1
      ).toString())
    );
  }
...
export class StoreItemBasket extends Component<IProductBasket> {
  ...
  constructor(
    ...
    this._index = container.querySelector(settings.page.basketItemIndex);
    this._price = container.querySelector(`.${blockName}__price`);
    this._button = container.querySelector(`.${blockName}__button`);
